...
$commonWords = "header|headers|failed|failure|required|requirement|denied|error|check|status|code|granted|missing|rejected|was|is|rule|rules|policy|policies|flow|attempt|request|requests|diagnostic|diagnostics|completed|endpoint|level|context|type|mode|info|information|parameter|param|params|service"
$paramHeaderRegex = [regex]"(?i)\b(authorization|proxy-authorization)(?::\s*|\s+(?!$commonWords\b))([A-Za-z0-9_.-]+)\s+([A-Za-z0-9_.-]+\s*=\s*(?:`"[^`"]*`"|'[^']*'|[A-Za-z0-9_.~+-]+)(?:\s*,\s*[A-Za-z0-9_.-]+\s*=\s*(?:`"[^`"]*`"|'[^']*'|[A-Za-z0-9_.~+-]+))*)"
$tokenHeaderRegex = [regex]"(?i)\b(authorization|proxy-authorization)(:\s*|\s+(?!$commonWords\b))([A-Za-z0-9_.-]+)\s+(?![A-Za-z0-9_.-]+\s*=\s*(?:`"[^`"]*`"|'[^']*'|[A-Za-z0-9_.~+-]+))([^\s,;]+)"
$paramSecretRegex = [regex]'(?i)\b(response|mac|signature|secret|password|token|api[_-]?key|key|credential|credentials|auth[_-]?token)\s*=\s*(?:"[^"]*"|''[^'']*''|[A-Za-z0-9_.~+-]+)'
function Redact-String([string]$value) {
    ...
    $value = $paramHeaderRegex.Replace($value, {
        param($m)
        $p = $m.Groups[0].Value.Substring(0, $m.Groups[0].Value.Length - $m.Groups[3].Value.Length)
        $params = $m.Groups[3].Value
        $redactedParams = $paramSecretRegex.Replace($params, '${1}=[REDACTED]')
        return $p + $redactedParams
    })
    $value = $tokenHeaderRegex.Replace($value, '${1}${2}${3} [REDACTED]')
