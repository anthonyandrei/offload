...
Before every automatic initial selection or quality escalation, the orchestrator defines the assignment and explicit constraints, discovers plausible candidates and tools, then opens the actual URL for the advisory benchmark reference that best matches the task and checks its latest relevant result for the exact model and reasoning effort before adjudicating candidates. This is benchmark-first routing. Treat the benchmark result as the primary quality evidence. Use judgment to interpret the benchmark result against runtime facts, not to bypass it; any departure requires a concrete runtime, availability, usage, pricing, or explicit-constraint reason and is recorded in the final report. Among candidates that appear capable of meeting the acceptance criteria, prefer the most accurate candidate as supported by the benchmark result. Compare its estimated assignment cost with the other capable candidates in the same benchmark context and skip it only when its price is a clear outlier among comparable capable candidates without a material quality advantage. Reasonable relative price is a judgment, not a fixed multiplier. Use current runtime or published pricing when available; benchmark cost is comparative evidence when current pricing is unavailable. If several benchmarks apply, open the best-matched task category first and keep other evidence secondary. Do not combine incomparable benchmarks or scores into a synthetic ranking. If the URL is inaccessible or has no relevant result, record an explicit evidence gap and continue without blocking launch. Unknown or stale evidence does not block launch. Confirmed exhaustion removes a candidate from automatic selection.

Honor explicitly named vendors, models, and reasoning efforts. Generic approval permits cross-vendor escalation. A named vendor constrains routing and escalation to that vendor. A named model or effort remains pinned for that dimension; remaining unpinned dimensions still follow benchmark-first routing. A fully pinned vendor, model, and effort has no automatic routing decision.
...
A quality-gate failure, such as unmet acceptance criteria, an unverified diff, or unsupported research citations, permits exactly one automatic escalation after reopening the actual benchmark URL, checking its latest relevant result, and refreshing usage checks. Choose the smallest credible improvement in quality that remains reasonably priced against the other capable candidates.
...
If cleanup fails, preserve the usable result and report a prominent cleanup warning with the exact path that remains. The final report states the assignment, selected vendor, model and effort, benchmark URL and access date, relevant benchmark result or explicit evidence gap, usage observations, launch facts, result, files or sources inspected, verification, escalation details, any departure reason, remaining uncertainty, failures or blockers, and cleanup status. Do not claim a worker ran until its launch succeeded.
